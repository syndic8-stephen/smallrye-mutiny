...
public class CodeGenHelper {
    ...
    public static String genConvParam(TypeInfo type, MethodInfo method, String expr) {
        ...
        if (isSameType(type, method)) {
            ...
            return "io.smallrye.mutiny.vertx.helpers.MutinyHelper.unwrap(" + expr + ")";
        } else if (type.isParameterized()) {
            ParameterizedTypeInfo parameterizedTypeInfo = (ParameterizedTypeInfo) type;
            if (kind == HANDLER) {
        ...
        if (typeVar.isClassParam()) {
            ...
            if (typeArg != null) {
                if (typeArg.isClassType()) {
                    return "io.smallrye.mutiny.vertx.helpers.TypeArg.of(" + typeArg.getParam().getName() + ")";
                } else {
        ...
        if (kind == OBJECT) {
            ...
            if (type.isParameterized()) {
                ...
                for (TypeInfo arg : parameterizedTypeInfo.getArgs()) {
                    ...
                    if (argKind == API) {
                        tmp.append("(io.smallrye.mutiny.vertx.helpers.TypeArg)").append(arg.getRaw().translateName(
                                AbstractMutinyGenerator.ID))
                                .append(".__TYPE_ARG");
                    } else {
                        String typeArg = "io.smallrye.mutiny.vertx.helpers.TypeArg.unknown()";
