...
public class ToSubscriberCodeWriter implements ConditionalCodeWriter {
    ...
    public void generate(ClassModel model, PrintWriter writer) {
        ...
        writer.format("  private io.smallrye.mutiny.vertx.helpers.WriteStreamSubscriber<%s> subscriber;%n",
    ...
    private void genToSubscriber(TypeInfo itemType, String fieldName, PrintWriter writer) {
        writer.format("  public synchronized io.smallrye.mutiny.vertx.helpers.WriteStreamSubscriber<%s> toSubscriber() {%n",
                genTypeName(itemType));
        ...
        if (itemType.getKind() == ClassKind.API) {
            ...
            writer.format("      %s = io.smallrye.mutiny.vertx.helpers.MutinyHelper.toSubscriber(getDelegate(), conv);%n",
                    fieldName);
            ...
            writer.format("      %s = io.smallrye.mutiny.vertx.helpers.MutinyHelper.toSubscriber(getDelegate());%n", fieldName);
        }
