...
* can receive 1 `item` event, or a `failure` event
* would not receive a `completion` event but can receive a `null` item
* do not handle `request` event
